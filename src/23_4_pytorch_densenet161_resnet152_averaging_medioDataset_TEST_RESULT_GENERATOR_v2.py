   ...
   # print("Testing started..")
    ...
    with torch.no_grad():
        for inputs, labels in test_dataloader:
            ...
            outputs1, outputs2 = test_model(inputs)
            outputs = (outputs1 + outputs2)/2
...
'''
    ...
    print("2. Precision score (PREC) =",
          mtc.precision_score(testset_labels, testset_predicted_labels, average="weighted"))
    print("3. Specificity (SPEC) =")
    print("4. Accuracy (ACC) =", mtc.accuracy_score(testset_labels, testset_predicted_labels, weights))
    print("5. Matthews correlation coefficient(MCC) =", mtc.matthews_corrcoef(testset_labels, testset_predicted_labels))
    ...
    f = open(os.path.join(history_dir, "23_2_cm_dictionary.pkl"), "wb")
...
def prepare_submission_file(image_names, predicted_labels, max_probability, time_per_image, submit_dir, data_classes):
    ...
    submission_dataframe.to_csv(os.path.join(submit_dir, "23_4_test2_prob_softmax"), index=False)
